...
ec_long %>% 
  ...
  theme_light() +
  labs(title = "Endometrial Cancer Incidence in Sweden (excluding Skåne Län, Stockholms Län, 
Västra Götalands Län), and All Ages from 1970-2024",
       x = "Year", y = "Incidence")
```
...
Is it significant, and how does it compare across other Nordic countries?
How do mortality rates compare in this age range as well?

```{r}
ec_long %>% 
  filter(age == "age_30_44",
         region != "Riket",
         region %in% large_regions) %>% 
  ggplot(aes(year, incidence)) +
  geom_line() +
  theme_light() +
  facet_wrap(vars(region)) +
  labs(title = "Endometrial Cancer Incidence among Swedish Women Ages 30-44 in Skåne Län, 
Stockholms Län, and Västra Götalands Län Regions from 1970-2024",
      x = "Year", y = "Incidence")
```
...
ec_long %>% 
  ...
  theme_light() +
  facet_wrap(vars(region)) +
  labs(title = "Endometrial Cancer Incidence among Swedish Women Ages 30-44 by Region
(excluding Skåne Län, Stockholms Län, Västra Götalands Län)",
      x = "Year", y = "Incidence")
```
